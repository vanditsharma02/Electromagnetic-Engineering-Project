...
for j = 1:(plates*2)
        ...
        if j == i
            ...
            if(j<=plates)
                ...
                for k = 1:gaussquad
                    for l = 1:gaussquad
                        x1 = [((mod(j-1-plates,NA)+1/2)*(B/NB)+u(k)) ((floor((j-1-plates)/NA)+1/2)*(A/NA)+u(l)) H];
                        x2 = [((mod(i-1,NA)+1/2)*(B/NB)) ((floor((i-1)/NA)+1/2)*(A/NA)) 0];
...
for j = 1:nx
    for k = 1:ny
        for i = 1:2*plates
            if i<= plates
                V(j,k) = V(j,k)+(1/(4*pie*eps))*Q(i)*((s(j,k)-X1(i))^2+(t(j,k)-Y1(i))^2+(H/2)^2)^(-0.5);
            else
                V(j,k) = V(j,k)+(1/(4*pie*eps))*Q(i)*((s(j,k)-X2(i-plates))^2+(t(j,k)-Y2(i-plates))^2+(H/2)^2)^(-0.5);
            end
        end
    end
end
...
for i = 1:plates
    ...
    u = u +(1/(4*pie*eps)).*(c-X1(i)).*Q2(i).*((c-X2(i)).^2+(d-Y2(i)).^2+(e-H).^2).^(-1.5);
end
v=0;
for i = 1:plates
    v = v +(1/(4*pie*eps)).*(d-Y1(i)).*Q1(i).*((c-X1(i)).^2+(d-Y1(i)).^2+(e).^2).^(-1.5);
    v = v +(1/(4*pie*eps)).*(d-Y2(i)).*Q2(i).*((c-X2(i)).^2+(d-Y2(i)).^2+(e-H).^2).^(-1.5);
end
w=0;
for i = 1:plates
    w = w +(1/(4*pie*eps)).*(e).*Q1(i).*((c-X1(i)).^2+(d-Y1(i)).^2+(e).^2).^(-1.5);
    w = w +(1/(4*pie*eps)).*(e-H).*Q2(i).*((c-X2(i)).^2+(d-Y2(i)).^2+(e-H).^2).^(-1.5); 
end
